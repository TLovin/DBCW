...
app.config['MYSQL_PASSWORD'] = 'M1o2v3i4e5s6!'
...
def task5():
    ...
    cur = mysql.connection.cursor()
    # Execute the search query
    query1 = "SELECT AVG(rating) FROM (SELECT *, @counter := @counter +1 AS counter FROM (SELECT DISTINCT ratings.userId, timestamp, rating FROM movie_db.ratings,movie_db.movies,movie_db.users WHERE movie_db.ratings.movieId = movie_db.movies.movieId AND movie_db.movies.title = %s AND movie_db.users.source = 'MovieLens' ORDER BY movie_db.ratings.timestamp ASC) as list, (select @counter:=0) AS initvar) AS X where counter <= (ROUND(10/100 * (SELECT COUNT( DISTINCT ratings.userId, timestamp) as movies FROM movie_db.ratings,movie_db.movies,movie_db.users WHERE movie_db.ratings.movieId = movie_db.movies.movieId AND movie_db.movies.title = %s AND movie_db.users.source = 'MovieLens'))) ORDER BY X.timestamp ASC"
    query2 = "SELECT AVG(rating) FROM (SELECT *, @counter := @counter +1 AS counter FROM (SELECT DISTINCT ratings.userId, timestamp, rating FROM movie_db.ratings,movie_db.movies,movie_db.users WHERE movie_db.ratings.movieId = movie_db.movies.movieId AND movie_db.movies.title = %s AND movie_db.users.source = 'MovieLens' ORDER BY movie_db.ratings.timestamp ASC) as list, (select @counter:=0) AS initvar) AS X where counter <= (ROUND(25/100 * (SELECT COUNT( DISTINCT ratings.userId, timestamp) as movies FROM movie_db.ratings,movie_db.movies,movie_db.users WHERE movie_db.ratings.movieId = movie_db.movies.movieId AND movie_db.movies.title = %s AND movie_db.users.source = 'MovieLens'))) ORDER BY X.timestamp ASC"
    query3 = "SELECT AVG(rating) FROM (SELECT DISTINCT ratings.userId, timestamp, rating FROM movie_db.ratings,movie_db.movies,movie_db.users WHERE movie_db.ratings.movieId = movie_db.movies.movieId AND movie_db.movies.title = %s AND movie_db.users.source = 'MovieLens') as selected"
    #cur.execute(f"SELECT * FROM rotten WHERE title = '{search_term}'")
    cur.execute(query1, (search_term,search_term))
    result1 = cur.fetchall() 
    cur.execute(query2, (search_term,search_term))
    result2 = cur.fetchall()
    cur.execute(query3, (search_term,))
